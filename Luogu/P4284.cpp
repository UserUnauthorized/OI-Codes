...
int main() {
	...
	std::function<void(valueType, valueType, realType)> addEdge = [&edge] (valueType a, valueType b, realType p) mutable {
	...
	DfsFunction dfs = [&edge, &P, &dp, &dfs] (int x, int from) mutable {
		...
		for(auto const &iter : edge[x]) {
			...
			dp[x] *= dp[to] + (1 - dp[to]) * (1 - iter.second);
		}
	};
	
	DfsFunction calc = [&edge, &dp, &ans, &calc] (int x, int from) mutable {
		for(auto const &iter : edge[x]) {
			int const to = iter.first;
			...
			ans[to] = 1 - dp[to] * ((1 - ans[x]) + ans[x] * (1 - iter.second));
