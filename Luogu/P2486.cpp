...
struct NODE {
    ...
    NODE operator+(const NODE &object) const {
        return NODE(this->right == object.left ? this->count + object.count - 1 : this->count + object.count,
                    this->left, object.right);
    }

    friend NODE merge(const NODE &left, const NODE &right) {
        return NODE(left.right == right.left ? left.count + right.count - 1 : left.count + right.count, left.left,
                    right.right);
    }
};
