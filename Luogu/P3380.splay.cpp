...
//		pre(current->value);

		{
			...
			if(tmp->leftSon != NULL){
				for(tmp = current->leftSon; tmp->rightSon != NULL; tmp = tmp->rightSon);
				this->splay(tmp);
			}
		}
...
public:
	...
	pointer build(const self::valueType &l, const self::valueType &r, const container &source){
		...
		current->leftSon = this->build(l, current->mid, source);
		current->rightSon = this->build(current->mid + 1, r, source);
