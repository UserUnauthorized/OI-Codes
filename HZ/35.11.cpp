...
//#endif
	...
	for(int i = 2; i <= N; ++i) {
		...
		for(int j = 1; j <= i; ++j) {
			...
			realType const ans_0_1 = dp[1][pre][j] + (1 - P[i - 1]) * distance(C[i - 1], C[i]) + P[i - 1] * distance(D[i - 1], C[i]);
			
			dp[0][now][j] = std::min(ans_0_0, ans_0_1);
			
			realType const ans_1_0 = (dp[0][pre][j - 1] + P[i] * distance(C[i - 1], D[i]) + (1 - P[i]) * distance(C[i - 1], C[i]));
			
			realType const ans_1_1 = P[i - 1] * (dp[1][pre][j - 1] + P[i] * distance(D[i - 1], D[i]) + (1 - P[i]) * distance(D[i - 1], C[i])) + 
									(1 - P[i - 1]) * (dp[1][pre][j - 1] + P[i] * distance(C[i - 1], D[i]) + (1 - P[i]) * distance(C[i - 1], C[i]));
