...
public:
	struct NODE {
		...
		void update(){
			...
			this->max = std::max((this->leftSon != nullptr ? (this->leftSon)->max : MIN), (this->rightSon != nullptr ? (this->rightSon)->max : MIN));
			this->min = std::min((this->leftSon != nullptr ? (this->leftSon)->min : MAX), (this->rightSon != nullptr ? (this->rightSon)->min : MAX));
