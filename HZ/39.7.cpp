...
int main() {
    ...
    for (int i = 1; i <= Q; ++i) {
        if (queryEdge[i << 1].LCA == -1)
            ...
            printf("%d\n", min(minWeight(queryEdge[i << 1].LCA, queryEdge[i << 1].from),
                               minWeight(queryEdge[i << 1].LCA, queryEdge[i << 1].to)));
    }
    return 0;
}
...
int minWeight(int up, int down) {
    if (deep[up] >= deep[down])
        return INT_MAX;
    int tk = log2(deep[down] - deep[up]);
    return min(way[tk][down], minWeight(up, fa[tk][down]));
}
...
void build() {
    ...
    const int count = cnt;
    cnt = 0;
    sort(sourceEdge + 1, sourceEdge + count);
