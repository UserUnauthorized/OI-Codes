...
//        std::cerr << "DEBUG";
        for (valueType i = 2; i <= size; ++i) {
            if (isPrime[i]) {
                ...
                data[i] = i + 1;
                minFactor[i] = i;
//                debug(i, isPrime[i], minFactor[i], data[i]);
            }
...
//                    if (data[minFactor[i]] == 0) {
//                        debug(i, t, iter, minFactor[i], minFactor[t]);
//                        debug(data[minFactor[i]], data[minFactor[t]]);
//
...
//        debug(newPos->first, newPos->second, a, A);
//        debug(std::prev(newPos)->first, std::prev(newPos)->second);
//        debug(std::next(newPos)->first, std::next(newPos)->second);
