...
int n, m, cnt(0);
int weight[maxn], dfn[maxn], out[maxn], nodeId[maxn], son[maxn], top[maxn], dep[maxn], father[maxn], size[maxn];
...
int main() {
    ...
    while (m--) {
        ...
        if (order == 1) {
            ...
            int x(read()), key(read());
            update(1, dfn[x], dfn[x] + size[x] - 1, key);
        } else {
            int x(read());
            long long result(0);
...
void build(int id, int l, int r) {
    ...
    if (l == r) {
        tree[id].sum = (long long) weight[nodeId[r]];
        return;
    }
