...
public:
    explicit Min_25(valueType n) : N(n), limit(std::floor(std::sqrt((long double) N))), ID(n), size(ID.size()),
                                   primeCount(0), lpf(size + 1, 0), G_1(size + 1, 0), G_2(size + 1, 0),
                                   F_prime(size + 1, 0) {
        sieve(limit);
        ...
        for (valueType k = 1; k <= primeCount; ++k) {
            ...
            for (valueType i = 1; ID[i] >= p2; ++i) {
                ...
                Dec(G_1[i], mul(p, sub(G_1[id], primeSum1[k - 1] + 1)));
                Dec(G_2[i], mul(mul(p, p), sub(G_2[id], primeSum2[k - 1] + 1)));
            }
        }

        for (valueType i = 1; i <= size; ++i)
            F_prime[i] = sub(G_2[i], G_1[i]);
    }
